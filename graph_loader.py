...
import numpy as np
import ast
from utils.graph_batcher import logger, GraphPairsBatchConstructor, load_pkl
...
class CustomDataLoader:
    key_kwargs = {"e1_key": "ligand", "e2_key": "protein", "label_key": "label", "series_key": "series"}
    ...
    def get_features(self, df_batch):
        ...
        e1_fp = torch.from_numpy(np.vstack(df_batch[self.key_kwargs["e1_key"]].map(self.e1_key_to_fp))).type(torch.float32).to(self.device)
        series_data = np.array(df_batch[self.key_kwargs["series_key"]].tolist())
        series_tensor = torch.tensor(series_data, dtype=torch.float32).to(self.device)
        y = torch.from_numpy(np.array(df_batch[self.key_kwargs["label_key"]])).type(torch.float32).view(-1, 1).to(self.device)
        return y, {"e1_graph": e1_graph, "e2_graph": e2_graph, "e1_fp": e1_fp, "series": series_tensor}


def load_data(dataset):
    df = pd.read_csv(f"data/{dataset}/updated_full.csv")
    df['series'] = df['series'].apply(ast.literal_eval)
    maxlen = 1000
    df['series'] = list(pad_sequences(df['series'].tolist(), maxlen=maxlen))
...
def pad_sequences(sequences, maxlen, value=0):
    """ Pad sequences to the same length with the given value. """
    padded_array = np.full((len(sequences), maxlen), fill_value=value, dtype=int)
